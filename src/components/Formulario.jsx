...
// Utilidad para previsualizar imágenes
...
export default function Form({ initial, onNext }) {
  ...
  // Cargar puntos cuando elijas una formación
  useEffect(() => {
    if (!selTitulo) return
  ...
  // Llamada a Netlify Function -> Pipedrive
  const rellenarDesdePipedrive = async () => {
    ...
    try {
      ...
      if (!r.ok) throw new Error(data?.error || 'Error al consultar Pipedrive')
            ...
            <div className="row g-3">
              <div className="col-md-6">
                ...
                <select className="form-select" value={selTitulo} onChange={(e)=>setSelTitulo(e.target.value)}>
                  ...
                  {Object.keys(plantillasBase).sort().map((t)=>(
                  ...
                  {(datos.contenidoTeorica || []).map((v, i) => (
                    <div className="input-group" key={`t-${i}`}>
                      ...
                      <button type="button" className="btn btn-outline-danger"
                        onClick={()=>setDatos(d=>({...d, contenidoTeorica:d.contenidoTeorica.filter((_,idx)=>idx!==i)}))}>
                        Eliminar
                      </button>
                    </div>
                  ))}
                ...
                <div className="d-grid gap-2">
                  {(datos.contenidoPractica || []).map((v, i) => (
                    <div className="input-group" key={`p-${i}`}>
                      ...
                      <button type="button" className="btn btn-outline-danger"
                        onClick={()=>setDatos(d=>({...d, contenidoPractica:d.contenidoPractica.filter((_,idx)=>idx!==i)}))}>
                        Eliminar
                      </button>
                    </div>
                  ))}
